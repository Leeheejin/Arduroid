...
Blockly.Blocks['blue_begin'] = {
  ...
  init: function() {
    ...
    this.setColour(330);
    this.appendDummyInput()
        .appendField(Blockly.Msg.ARD_BLUETOOTH_BEGIN)
        .appendField(
            new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.blueSerial), 'BLUE_SERIAL_ID')
        .appendField(Blockly.Msg.ARD_BLUETOOTH_BEGIN1)
        .appendField(
            new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.serialSpeed), 'SPEED')
        .appendField(Blockly.Msg.ARD_BLUETOOTH_BEGIN2);
    this.setInputsInline(true);
    this.setPreviousStatement(true, null);
    this.setNextStatement(true, null);
    this.setTooltip(Blockly.Msg.ARD_BLUETOOTH_BEGIN_TIP);
  },
  onchange: function(event) {                                           //���� ������� ������ �Ǿ����� �ʴ� ��� ��� ����
      if (!this.workspace || event.type == Blockly.Events.MOVE ||
          event.type == Blockly.Events.UI) {
          return;  // Block deleted or irrelevant event
      }

      // Get the Bluetooth Serial instance from this block
      var thisInstanceName = this.getFieldValue('BLUE_SERIAL_ID');
      // Iterate through top level blocks to find setup instance for the serial id
      var blocks = Blockly.mainWorkspace.getTopBlocks();
      var setupInstancePresent = false;
      for (var x = 0; x < blocks.length; x++) {
      ...
      if (!setupInstancePresent) {
        this.setWarningText(Blockly.Msg.ARD_SERIAL_PRINT_WARN.replace('%1',
            thisInstanceName), 'blue_setting');
      } else {
        this.setWarningText(null, 'blue_setting');
      }
    },                                                                                          //�������
  /**
   * Returns the serial instance name.
  ...
  getSerialSetupInstance: function() {
    return this.getFieldValue('BLUE_SERIAL_ID');
  },
  ...
  updateFields: function() {
    Blockly.Arduino.Boards.refreshBlockFieldDropdown(
        this, 'BLUE_SERIAL_ID', 'bluetooth');
    Blockly.Arduino.Boards.refreshBlockFieldDropdown(
        this, 'SPEED', 'blueserialSpeed');
  }
};
  ...
  init: function() {
    ...
    this.appendDummyInput()
        ...
        .appendField(
            new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.blueSerial), 'BLUE_SERIAL_ID')
                .appendField(Blockly.Msg.ARD_BLUETOOTH_AVAILABLE);
    this.setOutput(true, Blockly.Types.BOOLEAN.output);                //define return type
    this.setTooltip(Blockly.Msg.ARD_BLUETOOTH_BEGIN_TIP);
  },
  onchange: function(event) {
      if (!this.workspace || event.type == Blockly.Events.MOVE ||
  ...
  init: function() {
    ...
    this.appendDummyInput()
        .appendField(
            new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.blueSerial), 'BLUE_SERIAL_ID')
                .appendField(Blockly.Msg.ARD_BLUETOOTH_READ);
    this.setOutput(true, null);                 //define return type
    this.setTooltip(Blockly.Msg.ARD_BLUETOOTH_BEGIN_TIP);
  },
  onchange: function(event) {
      if (!this.workspace || event.type == Blockly.Events.MOVE ||
    ...
    this.appendDummyInput()
        .appendField(
            new Blockly.FieldDropdown(
                ...
                .appendField(Blockly.Msg.ARD_BLUETOOTH_WRITE1);
    this.setOutput(true, null);                 //define return type
    this.setTooltip(Blockly.Msg.ARD_BLUETOOTH_BEGIN_TIP);
  },
  onchange: function(event) {
  ...
  init: function() {
    ...
    this.appendDummyInput()
        .appendField(
            new Blockly.FieldDropdown(
                Blockly.Arduino.Boards.selected.blueSerial), 'BLUE_SERIAL_ID')
                .appendField(Blockly.Msg.ARD_BLUETOOTH_END);
           this.setPreviousStatement(true, null);
        this.setNextStatement(true, null);
    this.setTooltip(Blockly.Msg.ARD_BLUETOOTH_BEGIN_TIP);
  },
